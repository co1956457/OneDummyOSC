...
namespace OneDummy
{
    public partial class ComNiGift : Form
    {
        public ComNiGift()
        {
            InitializeComponent();
        ...
        private void buttonNiconamaGiftOSC_Click(object sender, System.EventArgs e)
        {
            // シリアライズ: フォーム情報から json 形式に整形したテキストを取得
            string jsonString = CommonNiconamaGiftSerialize();

            // デバッグ for debugging purposes
            // string path = @"C:\example\";
            // File.WriteAllText(path + "commonNiconamaGift.txt", jsonString);
        ...
        private string CommonNiconamaGiftSerialize()
        {
            // テキスト情報を読み取り JSON 化
