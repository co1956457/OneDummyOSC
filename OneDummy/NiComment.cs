...
namespace OneDummy
{
    public partial class NiComment : Form
    {
        public NiComment()
        {
            InitializeComponent();
        ...
        private void buttonNiconamaCommentOSC_Click(object sender, EventArgs e)
        {
            // シリアライズ: フォーム情報から json 形式に整形したテキストを取得
            string jsonString = NiconamaCommentSerialize();

            // デバッグ for debugging purposes
            // string path = @"C:\example\";
            // File.WriteAllText(path + "niconamaComment.txt", jsonString);
        ...
        private string NiconamaCommentSerialize()
        {
            ...
            string premiumText = "false";
            foreach (Control control in groupBoxPremium.Controls)
            {
                if (control is RadioButton radioButton && radioButton.Checked)
                {
                    // 選択されている RadioButton のテキストを取得
                    premiumText = radioButton.Text;
                    break; // 1つの選択された RadioButton が見つかればループを終了
                }
            }
